...
**Install `Pi` (the latest stable release from Bioconductor):**

```{r eval=FALSE}
source("http://bioconductor.org/biocLite.R")
# to use the latest version of Bioconductor, please also run: biocLite("BiocUpgrade") 
biocLite("Pi")
```
...
```{r fig.width=1.8, fig.height=1.8, fig.align="center", echo=FALSE, eval=TRUE}
library(png)
library(grid)
...
## xPredictROCR and xPredictCompare
...
- *[`xPierMatrix`](http://rawgit.com/hfang-bristol/Pi/master/inst/xPierMatrix.html)*: extracts priority matrix from a list of `pNode` objects, in which rows are genes and columns for the predictors (corresponding to the `pNode` objects).
...
mp <- xPierManhattan(pNode, top=25, y.scale="sqrt", RData.location=RData.location)
print(mp)
```
```{r, eval=FALSE, echo=FALSE}
png(file="saved.Pi.gene_manhattan.png", height=480, width=480*2, res=96*1.2)
print(mp)
dev.off()
```
